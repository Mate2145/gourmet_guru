...
class AddRecipeController extends GetxController {
  ...
  bool addRecipe(){
    ...
    analyzedInstructions.add(new AnalyzedInstructions(name: "", steps: this.steps.value));

    if(cookTime.value == 0){
      Get.snackbar(
      "Error", "Read in minutes is 0",
    ...
    final detailedRecipe = new RecipeDetailed(id: id, title: this.recipeName.value, summary: this.summary.value, addRecipedishType: this.selectedMealType.value.name, servings: this.servings.value.toInt(), readyInMinutes: this.cookTime.value.toInt(), extendedIngredients: listofIngredients,instructionList: analyzedInstructions);
